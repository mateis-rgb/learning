...
const int Capteurs_IR1 = PIN_A2; // Capteur infrarouge gauche (Analog)
const int Capteurs_IR2 = 2; // Capteur infrarouge centre (Digital)
const int Capteurs_IR3 = PIN_A3; // Capteur infrarouge droit (Analog)
const int Capteur_Sol = PIN_A0;  // Détecteur de sol
// const int Capteur_Sol2 = 16; // Deuxième détecteur de sol (si nécessaire)
...
unsigned char Detecter_Ligne_Blanche(void)
{
    ...
    int sol = digitalRead(Capteur_Sol);  // Le capteur de sol est en LOW pour une ligne blanche
...
void Robot_Avance(void)
{
    analogWrite(PWMD, 150);  // Vitesse du moteur droit
    analogWrite(PWMG, 143);  // Vitesse du moteur gauche
...
void Robot_Tourner_Gauche(void)
{
    analogWrite(PWMD, 133);  // Vitesse du moteur droit
    analogWrite(PWMG, 150);  // Vitesse du moteur gauche
    digitalWrite(SENSG, LOW); // Moteur gauche en arrière
    digitalWrite(SENSD, LOW);  // Moteur droit en avant
    delay(45); // Ajuster la durée du virage si nécessaire
...
{
    analogWrite(PWMD, 150);  // Vitesse du moteur droit
    analogWrite(PWMG, 130);  // Vitesse du moteur gauche
    digitalWrite(SENSG, HIGH); // Moteur gauche en avant
    digitalWrite(SENSD, HIGH);  // Moteur droit en arrière
    delay(65); // Ajuster la durée du virage si nécessaire
    Arret_Moteur();  // Arrêter les moteurs après le virage
}
