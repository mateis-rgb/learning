...
void loop(void)
{
  ...
  if (val_alt != val){
    ...
    // erase the old line
    DrawLine(160, 120, 160+100*cos(val_rad_alt), 120+100*sin(val_rad_alt), TFT_BLACK);  // the line itself
    DrawLine(160+100*cos(val_rad_alt), 120+100*sin(val_rad_alt), 160+100*cos(val_rad_alt)-10*cos(val_rad_alt+pi_quarter), 120+100*sin(val_rad_alt)-10*sin(val_rad_alt+pi_quarter), TFT_BLACK);  // the arrow line one side
    DrawLine(160+100*cos(val_rad_alt), 120+100*sin(val_rad_alt), 160+100*cos(val_rad_alt)-10*cos(val_rad_alt-pi_quarter), 120+100*sin(val_rad_alt)-10*sin(val_rad_alt-pi_quarter), TFT_BLACK);  // the arrow line one side
    // draw new line
    ...
    DrawBox(???, ???, ???, ???, ???);
    ILI9341SetCursor(??, ??);
    DrawChar(5, LargeFont, TFT_WHITE);
    itoa(val,cstr,10);
    DrawString(cstr, LargeFont, TFT_WHITE);
    // swap values
    val_rad_alt=val_rad;
    val_alt=val;
  }
