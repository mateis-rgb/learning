...
//   computes PAT
//   Arduino Nano
//   Angers sequence readout potentiometer
//-----------------------------------------------------------------------------
...
int posx_max=320; //max position x
int posy=0; //start position y
int posy_max=240;  //max position y
...
void loop(void)
{
  ...
  val_rad=val;
  val_rad=val_rad/100;
  DrawBox(50, 100, 70, 50, TFT_MAROON);
  ILI9341SetCursor(65, 125);
  DrawChar(5, LargeFont, TFT_WHITE);
  val_display=120*sin(val_rad);
  char cstr[16];
  itoa(val_display,cstr,10);
